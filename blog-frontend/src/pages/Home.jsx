...
const Home = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [postRes, catRes, contributorRes] = await Promise.all([
          axios.get("/posts"),
          axios.get("/categories"),
          axios.get("/posts/top-contributors"),
        ]);

        const posts = postRes.data.data.posts || [];
        const categories = catRes.data.data.categories || [];
        const contributors = contributorRes.data.data || [];
    ...
    <div className="bg-[#f9f9f9] font-sans text-gray-800">
      ...
      >
        <div className="max-w-7xl w-full grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
          ...
          {/* Right Accent Column */}
          <div className="flex flex-col gap-6 text-right items-end lg:items-end sm:items-center sm:text-center sm:w-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 px-4 sm:px-6 lg:px-12 py-12">
        ...
        {/* Sidebar */}
        <div className="lg:col-span-3 space-y-8 sticky top-28 self-start">
          {/* Categories */}
          {/* Top Contributors */}
          <motion.div
          ...
          >
            ...
            <ul className="space-y-4">
              {topContributors.length === 0 ? (
                <li className="text-sm text-gray-500">No contributors yet</li>
              ) : (
                topContributors.map((contributor, i) => (
                  <li key={contributor.userId} className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 bg-gray-500 text-white rounded-full flex items-center justify-center text-sm font-semibold uppercase">
                        {contributor.name?.[0] || "U"}
                      </div>
                      <div>
                        <p className="text-gray-800 font-medium">{contributor.name}</p>
